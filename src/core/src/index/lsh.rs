...
impl<'a, L> Index<'a> for LSHIndex<L>
where
    ...
    SigStore<L>: ReadData<L>,
{
    ...
    pub fn from_path<P: AsRef<Path>>(path: P) -> Result<LSHIndex<L>, Error> {
        unimplemented!();
    }

    pub fn from_reader<R, P>(rdr: &mut R, path: P) -> Result<LSHIndex<L>, Error>
    where
        R: Read,
        P: AsRef<Path>,
    {
